...
export const BuildInPublicSection = () => {
  ...
  useEffect(() => {
    ...
    const scroll = () => {
      // Only auto-scroll when it overflows (e.g. mobile view)
      if (el.scrollWidth > el.clientWidth) {
        if (!isPaused) {
          scrollPos.current += 0.35; // Slower speed of the loop for a smoother premium feel
          
          if (midPointRef.current) {
            // px-8 is 32px padding, so we subtract it to find the exact reset point
            const resetPoint = midPointRef.current.offsetLeft - 32;
            if (scrollPos.current >= resetPoint) {
              scrollPos.current -= resetPoint;
            }
          }
          el.scrollLeft = scrollPos.current;
        } else {
          // Sync position when user manually drags
  ...
  // Duplicate reels for infinite loop
  ...
  return (
    <section className="py-20 lg:py-28 w-full flex flex-col items-center justify-center bg-transparent z-10 overflow-hidden relative">
      ...
      <div className="w-full max-w-[1200px] mx-auto px-4 flex flex-col items-center relative z-10">
        ...
        <div className="text-center flex flex-col items-center mb-8 relative z-20">
          ...
          <motion.div
            ...
            className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 justify-center mt-6"
          >
            <a 
              href="https://instagram.com/builderstudio.in"
              target="_blank"
              rel="noopener noreferrer"
              className="group flex items-center justify-center gap-2 sm:gap-3 w-full sm:w-auto h-[48px] sm:h-[56px] px-6 sm:px-8 rounded-xl bg-white hover:bg-neutral-200 text-black font-semibold text-[14px] sm:text-[16px] transition-all duration-300 hover:-translate-y-1 shadow-[0_4px_20px_rgba(255,255,255,0.1)] hover:shadow-[0_8px_30px_rgba(255,255,255,0.2)]"
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="sm:w-[20px] sm:h-[20px]">
                <defs>
              ...
              Follow Our Journey
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="ml-1 group-hover:translate-x-1 transition-transform sm:w-[18px] sm:h-[18px] w-[16px] h-[16px]"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
            </a>
            ...
            <button 
              ...
              className="inline-flex items-center justify-center gap-2 w-full sm:w-auto h-[48px] sm:h-[56px] px-6 sm:px-8 rounded-xl bg-[#10B981] text-black font-bold text-[14px] sm:text-[16px] hover:bg-[#10B981]/90 hover:-translate-y-1 hover:shadow-[0_4px_20px_rgba(16,185,129,0.3)] hover:shadow-[0_8px_30px_rgba(16,185,129,0.4)] transition-all duration-300 group"
            >
              Build Your MVP Next
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="ml-1 group-hover:translate-x-1 transition-transform sm:w-[18px] sm:h-[18px] w-[16px] h-[16px]"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
            </button>
          </motion.div>
      ...
      <div 
        ...
        className="w-full relative mt-4 mb-10 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] will-change-transform will-change-scroll"
      >
        <div className="flex md:justify-center items-center gap-4 sm:gap-6 md:gap-8 px-8 md:px-4 w-max md:w-full mx-auto min-w-max md:min-w-0 py-8 md:py-32">
          {duplicatedReels.map((reel, index) => {
            const originalIndex = index % 7;
            const offset = originalIndex - 3; // -3, -2, -1, 0, 1, 2, 3
            
            // No rotation, purely vertical staggered layout for 7 cards on desktop
            let translateClass = "md:translate-y-[60px]";
            if (Math.abs(offset) === 3) translateClass = "md:-translate-y-[120px]";
            else if (Math.abs(offset) === 2) translateClass = "md:-translate-y-[60px]";
            else if (Math.abs(offset) === 1) translateClass = "md:translate-y-0";

            const zIndex = 40 - Math.abs(offset) * 10;
            const isDuplicate = index >= 7;
            
            return (
              <div 
                key={`${reel.id}-${index}`}
                ref={index === 7 ? midPointRef : null}
                className={`relative shrink-0 w-[42vw] sm:w-[120px] md:w-[150px] lg:w-[180px] aspect-[9/16] rounded-[12px] md:rounded-[18px] bg-[#0A0A0A]/80 backdrop-blur-xl border border-white/[0.08] overflow-hidden group cursor-pointer transition-colors duration-300 hover:border-white/20 ${translateClass} ${isDuplicate ? 'md:hidden' : ''}`}
                style={{ 
                  zIndex
                }}
              >
                {/* Real Thumbnail Image */}
                <img src={reel.image} alt={reel.title} loading="lazy" className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-90 transition-opacity duration-500" />
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 md:w-5 md:h-5 flex items-center justify-center">
                      <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <defs>
                          <linearGradient id={`insta-${reel.id}-${index}`} x1="2" y1="22" x2="22" y2="2" gradientUnits="userSpaceOnUse">
                            <stop stopColor="#FEDA77"/>
                            <stop offset="0.25" stopColor="#F58529"/>
                            <stop offset="0.5" stopColor="#DD2A7B"/>
                            <stop offset="0.75" stopColor="#8134AF"/>
                        ...
                        <path fill={`url(#insta-${reel.id}-${index})`} d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm3.96-9.155a1.44 1.44 0 11-2.88 0 1.44 1.44 0 012.88 0z"/>
                      </svg>
                    </div>
